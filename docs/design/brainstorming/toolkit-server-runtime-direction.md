...
| Service Host | generated Decode와 입력 middleware, 서비스 등록 편의, handler 실행 전달과 응답 Encode·출력 연결 |
| WorldRuntime | 입력 수집, tick, ECS system 실행, 상태 변경 확정과 결과 출력을 관리하는 재사용 실행 기반 |
| 공용 실행 계층(가칭) | 기존 header-only Common과 분리하여 runtime들이 실제로 공유할 작업·실행 도구를 제공 |
...
- NetworkRuntime과의 연결은 Host Input Adapter와 Host Output Adapter로 나눈다. 특정 PrivateServer API를 Host 계약으로 삼지 않으며, 역할 분리가 별도 스레드·큐를 각각 만든다는 뜻은 아니다.
- 진입점에서 서비스와 executor를 명시적으로 등록하고, 매크로로 패킷과 handler의 연결 선언을 줄인다. 프로젝트 전체 서비스 자동 탐색·등록은 첫 범위에서 제외한다.
...
- Host는 요청 데이터와 handler 호출 작업을 move 기반으로 넘기는 방향을 사용하고, World가 실행 시점과 순서를 정한다. 후처리 middleware와 범용 RPC 자동 호출 관리는 현재 Host 범위에서 제외하되, 기본 수명·거절·종료 처리와 안전한 응답 송신은 유지한다.
- 공용 실행 도구는 첫 Host 경로에 필요한 만큼만 마련한다. Akka의 mailbox/job queue 구조를 참고하되 MPSC·thread pool·spin lock·메모리 풀을 모두 선행 구현하지 않는다. Nakama의 match/tick 모델은 WorldRuntime 설계 때 참고한다.
...
- Service Host와 WorldRuntime 사이 executor의 구체적인 수락·거절·데이터 수명과 World 내부 실행 규칙
...
[Issue #2의 Service Host 설계](../issue-2-typed-service-host.md)에 최신 로컬 합의와 tracker 미동기화 상태를 기록한다. WorldRuntime과 NetworkRuntime 이관은 별도의 후속 주제로 남기며, 이 메모만으로 두 runtime의 구현이나 GitHub Issue 변경을 승인하지 않는다.

...
`TkResult`를 반환하는 함수는 실패 시 output object와 output buffer를 변경하지 않는 failure atomicity를 기본 계약으로 사용한다. Decode는 임시 객체를 거쳐 commit하고 Encode는 모든 검증 후 기록하며, 필요 크기와 같은 정보성 output을 실패에서도 갱신하는 예외는 각 API가 명시해야 한다.

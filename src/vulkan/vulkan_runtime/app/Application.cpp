...
{
    Impl(const ApplicationConfig &config, IGame &game) : game_{game}
    {
        if (config.shaderDirectory == nullptr || config.shaderDirectory[0] == '\0')
        {
            throw std::invalid_argument("Shader directory must not be null or empty");
        }
...
Application::Application(const ApplicationConfig &config, IGame &game) : impl_{new Impl(config, game)}
{
}
...
void Application::Impl::Run()
{
    ...
    SwapchainSupport swapchainSupport = Swapchain::QuerySwapchainSupport(selection.physicalDevice, surfaceHandle.Get());
    ...
    Renderer2D renderer(selection.physicalDevice, context.GetDevice(), swapchainSetting.surfaceFormat.format,
                        FrameResources::FramesInFlight(), kMaxInstances, shaderDirectory_);
    ...
    try
    {
        ...
        while (running)
        {
            ...
            if (recreateSwapchain || window.HasFramebufferResized())
            {
                ...
                std::unique_ptr<Swapchain> newSwapchain =
                    std::make_unique<Swapchain>(context.GetDevice(), surfaceHandle.Get(), swapchainSetting, selection,
            ...
            game_.Update();
            renderer.UpdateDrawData(currentFrameIndex, game_.GetDrawData());

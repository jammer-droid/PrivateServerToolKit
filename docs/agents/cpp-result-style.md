...
상세 정보를 보존하기 위해 모듈별 result 타입을 만들거나 모든 세부 원인을 `TkResult` 값으로 추가하지 않는다. Tool별 diagnostic callback이나 포맷도 만들지 않으며 common 계층의 Diagnostic과 `TkDiagnosticCallbackInfo` 계약을 사용한다.
